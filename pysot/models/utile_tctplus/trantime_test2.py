...
from pysot.models.utile_tctplus.multi_head_attention_topk import MultiHeadAttentionTopK
...
class Transformertime(Module):
    ...
    def _reset_parameters(self):
        r"""Initiate parameters in the transformer model.初始化参数"""
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=384, dropout=0.1, activation="relu"):
        ...
        self.self_attn0 = MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, src: Tensor, srcc: Tensor, src_mask: Optional[Tensor] = None,
        ...
        src0 = self.self_attn1(src, src, src, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]
        srcs0=src + self.dropout0(src0)
        srcs0 = self.norm0(srcs0)

        src1 = self.self_attn1(srcc, srcs0, srcs0, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]
        srcs1 = srcs0 + self.dropout1(src1)
        ...
        src = self.modulation(srcs2.view(b, c, int(s ** 0.5), int(s ** 0.5))\
                              , srcs1.contiguous().view(b, c, int(s ** 0.5), int(s ** 0.5))).view(b, c, -1).permute(2,
                                                                                                                    0,
                                                                                                                    1)
...
class TransformerDecoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=384, dropout=0.1, activation="relu"):
        ...
        self.self_attn = MultiHeadAttentionTopK(d_model, nhead, dropout=dropout, top_k=16)
